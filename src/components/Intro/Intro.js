  ...
  return (
    <Container className='intro'>
      ...
      <m.div initial={{opacity: 0}} animate={{opacity: 1}} transition={{ opacity: {duration: 0.5, delay: 1.75}}} style={{textAlign: 'center'}}>
        ...
        <Link to='/me' onClick={(e) => { dispatch({type: 'jump', payload: 2})}}><m.div className='button' whileHover={{backgroundColor:"#00a8a8", scale:1.1}} transition={{scale: {type:'spring', stiffness: 500, damping: 30}}}>Get in touch</m.div></Link>
      </m.div>
    </Container>
  )
}